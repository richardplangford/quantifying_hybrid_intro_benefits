...
global lastf lastgrad lasttheta delta tol cdid cdindex cdindexstart N draws T estbetamu ...
    ...
    X IV W xv xz priceinfo dpriceinfoprice incomebrackets compete price ...
    population outshare Omega var_cov ddeltatheta var_cov_delta var_cov_beta
tic
if theta==lasttheta
    f = lastf;
    grad = lastgrad;
...
for i=1:numbetaXZ
    ...
    dG2theta(:,i) = sum(bsxfun(@times,popweight,dG2temptheta),1)';
...
for i=1:numalpha
    ...
    dG3theta(:,i) = sum(bsxfun(@times,popweight,dG3temptheta),1)';
...
Gtemp = [G1temp,G2temp,G3temp];
Omega = (Gtemp'*Gtemp)./cdindex(T);

var_cov = (dGtheta'/W*dGtheta)\(dGtheta'/W*Omega/W'*dGtheta)/(dGtheta'/W*dGtheta);
var_cov_delta = ddeltatheta*var_cov*ddeltatheta';
var_cov_beta = (X'*P*X)\(X'*P*var_cov_delta*P'*X)/(X'*P*X)';
