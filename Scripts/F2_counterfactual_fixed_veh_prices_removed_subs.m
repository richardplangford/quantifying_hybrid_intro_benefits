% F0_counterfactual: calculates the compensating variation for all new
...
fed_sub = zeros(cdindex(T),3);
fed_sub = fed_sub + [X(:,11)<=5 , X(:,11)==6, X(:,11)==7];
fed_sub(:,1) = fed_sub(:,1)*500;
fed_sub(:,2) = fed_sub(:,2)*(.75*3150+.25*1575);
fed_sub(:,3) = fed_sub(:,3)*(.25*1575+.5*787.5);
fed_sub_agg = sum(fed_sub,2);

price = price + X(:,10).*fed_sub_agg/10000;
...
save('counterfactuals_fixed_veh_prices_removed_subs.mat');
