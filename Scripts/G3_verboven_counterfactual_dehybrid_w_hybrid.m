...
load('verboven_reynaert_full_estimates.mat');

rblastn = maxNumCompThreads(2);

% Set up model with estimated coefficients
betaU = verboven_reynaert_full_esttheta(1:numbetaU);
betaO = verboven_reynaert_full_esttheta(numbetaU+1:numbetaU+numbetaXZ);
alpha = verboven_reynaert_full_esttheta(numbetaU+numbetaXZ+1:numbetaU+numbetaXZ+numalpha);
...
% Remove all hybrid functionality from the market; adjust data accordingly
price_orig = price;
delta_orig = delta;
Xreal = X;
Xtrans = X';
...
% Loads the adjusted MPGs
load('prepped_data_dehybrid.mat');
% Continues in the same way as the hybrid characteristic removal CF, but
% leaves in hybrid indicators
...
compete = compete(X(:,18)==0,Xtransdehybrid(18,:)==0);
C = C(X(:,18)==0,:);
...
while norm > tol && avgnorm > 1e-3*tol && i < 1000
    ...
    new_delta = [X(:,1:13),new_price]*estbetamu + xihat;
...
save('counterfactuals_dehybrid_w_hybrid.mat');
